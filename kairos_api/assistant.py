...
SYSTEM_PROMPT = (
    ...
    "2026-07-07, not yet measured in our data and its rating basis not modeled in the engine, "
    "sourced to docs/quarter-hour-billing.md. In this market settlement is PER SPOT: a spot's "
    ...
    "whole break sat inside one quarter hour. The optimal position relative to a boundary need "
    "not be symmetric straddling: it depends on when viewers actually leave after a break "
    "starts and when they return after content resumes, any placement must still respect "
    "programme content constraints, and a single quarter or half hour often holds two or more "
    "breaks that share the same windows. Two currencies: the engine's retention measurement is "
    ...
    "this caveat and flag it as an owner-stated convention not yet measured, so you do not "
    "overclaim."
)
