...
// count is null when the server did not report it, never invented.
export function readEventsLayer(state) {
  if (!state || typeof state !== 'object') return { supported: false, enabled: false, count: null };
  ...
  if (!source && !('events' in activation)) return { supported: false, enabled: false, count: null };
  const enabled = Boolean(source ? (source.enabled ?? source.active ?? activation.events) : activation.events);
  const countRaw = source ? (source.active_event_count ?? source.active_events ?? source.event_count ?? source.count) : undefined;
  const count = Number.isFinite(Number(countRaw)) && countRaw !== undefined && countRaw !== null ? Number(countRaw) : null;
  return { supported: true, enabled, count };
}
