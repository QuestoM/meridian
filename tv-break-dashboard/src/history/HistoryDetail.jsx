...
function DeltaCell({ field, unit, locale }) {
  ...
  const sign = value > 0 ? '+' : '-';
  return <span className={`hist-delta ${value > 0 ? 'up' : 'down'}`}><Figure>{`${sign}${sized(Math.abs(value), unit, locale)}`}</Figure></span>;
}
...
function RunDetail({ entry, locale }) {
  ...
  if (state.status === 'error') return <p className="hist-empty warn">{pageText(locale, `The run record could not be read. ${state.error}`, `לא ניתן לקרוא את רשומת ההרצה. ${state.error}`)}</p>;
  ...
  return (
    <div className="hist-run">
      <div className="hist-detail-line">
        ...
        <span>{scope || pageText(locale, 'The whole saved plan', 'כל התוכנית השמורה')}</span>
      </div>
      <div className="hist-detail-line">
      ...
      <div className="hist-detail-line">
        ...
        <code><Code>{run.run_id}</Code></code>
      </div>

      <h4 className="hist-detail-h">{pageText(locale, 'What this run produced', 'מה ההרצה הזו הפיקה')}</h4>
      <p className="hist-basis">{pageText(locale, `Every figure below is this run's own recorded outcome over ${scope}.`, `כל מספר כאן הוא התוצאה שההרצה עצמה רשמה, על ${scope}.`)}</p>
      <div className="hist-run-grid">
        ...
        {RUN_FIELDS.map(([key, en, he, unit]) => (
          <div className="hist-run-row" key={key}>
            <span>{pageText(locale, en, he)}</span>
            <Figure>{fieldValue(key === 'segment_count' ? run.segment_count : summary[key], unit, locale)}</Figure>
      ...
      <div className="hist-kv">
        {Object.entries(run.inputs || {}).map(([name, checksum]) => (
          <div className="hist-detail-line" key={name}>
            <span className="hist-detail-key"><Code>{name}</Code></span>
            <code><Code>{checksum || pageText(locale, 'The file was not there', 'הקובץ לא היה שם')}</Code></code>
          </div>
        ))}
      </div>
      ...
      <div className="hist-kv">
        {Object.entries({ ...(run.guardrails || {}), ...(run.assumptions || {}) }).map(([name, value]) => (
          <div className="hist-detail-line" key={name}>
            <span className="hist-detail-key">{forceLabel(name, locale)}</span>
            <Name>{forceValue(name, value, locale)}</Name>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="hist-kv">
      <div className="hist-detail-line">
        ...
        <span>{actLabel(facts.action, outcome, locale)}</span>
      </div>
      {members.length > 1 ? (
        <div className="hist-detail-line">
          <span className="hist-detail-key">{pageText(locale, 'How many', 'כמה')}</span>
          <span>{pageText(locale, `${members.length} of these in a row, from ${stampLabel(entry.oldestTs, locale)} to ${stampLabel(entry.ts, locale)}.`, `${members.length} ברצף, מ-${stampLabel(entry.oldestTs, locale)} עד ${stampLabel(entry.ts, locale)}.`)}</span>
        </div>
      ) : null}
      <div className="hist-detail-line">
        <span className="hist-detail-key">{pageText(locale, 'Request', 'בקשה')}</span>
        <code><Code>{`${facts.method || ''} ${facts.path || ''}`.trim()}</Code></code>
      </div>
      <div className="hist-detail-line">
        ...
        <span className="hist-result">
          ...
          {Number.isFinite(status) ? <code><Code>{status}</Code></code> : null}
        </span>
      </div>
      <div className="hist-detail-line">
        <span className="hist-detail-key">{pageText(locale, 'Took', 'ארך')}</span>
        <Figure>{facts.duration_ms === null || facts.duration_ms === undefined ? pageText(locale, 'Not recorded', 'לא נרשם') : `${formatNumber(facts.duration_ms, locale)} ms`}</Figure>
      </div>
      {outcome !== APPLIED ? (
        <p className="hist-note warn">{outcomeNote(outcome, locale)}</p>
      ) : null}
      {preview && outcome === APPLIED ? (
        <p className="hist-note">{pageText(locale, 'Nothing was saved. This act computed an answer from the saved state and left it exactly as it was, so there is nothing here to put back.', 'לא נשמר דבר. הפעולה חישבה תשובה מהמצב השמור והשאירה אותו בדיוק כפי שהיה, ולכן אין כאן מה להחזיר.')}</p>
      ) : null}
      <p className="hist-note">{pageText(locale, 'The recorder stores metadata only: no request body, no query string and no credential has ever entered this record.', 'הרישום שומר נתוני-על בלבד: גוף הבקשה, מחרוזת השאילתה ופרטי הזדהות מעולם לא נכנסו לרשומה הזו.')}</p>
      {members.length > 1 ? (
        <div className="hist-members">
          ...
          {members.map((member) => (
            <div className="hist-member" key={member.id}>
              <time><Figure>{stampLabel(member.ts, locale)}</Figure></time>
              <code><Code>{`${(member.facts || {}).method || ''} ${(member.facts || {}).path || ''}`.trim()}</Code></code>
              <Figure>{(member.facts || {}).status}</Figure>
            </div>
          ))}
        </div>
      ) : null}
...
function SignInDetail({ entry, locale }) {
  ...
  return (
    <div className="hist-kv">
      <div className="hist-detail-line">
        <span className="hist-detail-key">{pageText(locale, 'What', 'מה')}</span>
        <span>{pair(SIGN_IN_LABELS, facts.event, locale)}</span>
      </div>
      <div className="hist-detail-line">
        <span className="hist-detail-key">{pageText(locale, 'Role', 'תפקיד')}</span>
        <span>{facts.role || pageText(locale, 'Not recorded', 'לא נרשם')}</span>
      </div>
    </div>
  );
}
    ...
    <aside className="card hist-detail" aria-label={pageText(locale, 'Entry detail', 'פרטי הרשומה')}>
      <header className="hist-detail-head">
        <div>
          ...
          <span className="hist-detail-actor" title={actorHint(entry.actor, locale)}><Name>{actorLabel(entry.actor, locale)}</Name></span>
          <time className="hist-detail-time"><Figure>{stampLabel(entry.ts, locale)}</Figure></time>
        </div>
      ...
      {actorHint(entry.actor, locale) ? (
        <p className="hist-note hist-actor-note">{actorHint(entry.actor, locale)}</p>
      ) : null}
