// Plain language for who the agreement covers and what each side undertook:
// identity and scope (A), the advertiser's commitments (D), and the channel's
// guarantees and make-good mechanics (E).
//
// Same {headline, rows, table} contract as term-language-money.js. Free text the
// schema stores is carried through as a quoted row rather than paraphrased,
// because on a make-good rule the exact wording is the operative fact.
//
// Placement, process and measurement (F, G, H) live in
// term-language-placement.js and are merged into TERM_DESCRIBERS at the bottom,
// so every caller still resolves one table by term id.
...
import { PLACEMENT_DESCRIBERS } from './term-language-placement';

// Identity and scope (A), what the advertiser undertakes (D), and what the
// channel guarantees and how it makes good (E). The placement, process and
// measurement families live in term-language-placement.js and are merged in
// below, so every caller still resolves one describer table by term id.
const COMMITMENT_DESCRIBERS = {
...
export const TERM_DESCRIBERS = { ...COMMITMENT_DESCRIBERS, ...PLACEMENT_DESCRIBERS };
