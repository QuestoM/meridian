...
The per-group fingerprint helpers live in
:mod:`kairos.export._freshness_groups` (split to keep this module under the
project line limit) and are re-exported here so import paths are stable.
"""
...
from kairos.export._freshness_groups import (  # noqa: F401 - re-exported names
    ABSENT,
...
def schedule_input_fingerprints(root: str | Path) -> dict[str, str]:
    ...
    constants, never hardcoded):
      ...
      * ``events``       the calendar-events store (``DEFAULT_EVENTS_PATH``),
                         counted ONLY while the event pricing layer
                         (``pricing_activation.events``) is active; with the
    ...
    # events: the calendar-events store, ONLY while pricing_activation.events is
    # on. Off (the shipped default) the engine never reads the store, so the
    # group is omitted and the sidecar is byte-identical to the pre-events stamp.
