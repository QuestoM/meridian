...
First operator account: the auth store lives on EFS
(`/app/state/data/auth/users.json`); seed it once through ECS exec with the
app's own auth_store module, then the login screen owns everything after.
